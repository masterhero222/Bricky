...
let WorkersController = class WorkersController {
    ...
    async getProfile(req) {
        if (req.user.role !== 'worker') {
            throw new common_1.BadRequestException('Не сте майстор');
        }
        return this.workersService.findById(req.user.id);
    }
    async updateProfile(body, req) {
        if (req.user.role !== 'worker') {
            throw new common_1.BadRequestException('Не сте майстор');
        }
        return this.workersService.updateProfile(req.user.id, body);
    }
};
...
__decorate([
    ...
    __metadata("design:returntype", Promise)
], WorkersController.prototype, "updateProfile", null);
