...
export class MailService {
  ...
  async sendEmailVerification(payload: { email: string; name?: string; token: string }) {
    ...
    return this.sendAccountMail({
      ...
      text: `Здравей${payload.name ? `, ${payload.name}` : ''}!\n\nПотвърди имейла си от този линк:\n${verificationUrl}\n\nАко не си създавал акаунт в Bricky, игнорирай това съобщение.`,
      html: `<p>Здравей${payload.name ? `, ${payload.name}` : ''}!</p><p>Потвърди имейла си от този линк:</p><p><a href="${verificationUrl}">${verificationUrl}</a></p><p>Ако не си създавал акаунт в Bricky, игнорирай това съобщение.</p>`,
  ...
  async sendPasswordReset(payload: { email: string; name?: string; token: string }) {
    ...
    return this.sendAccountMail({
      ...
      html: `<p>Здравей${payload.name ? `, ${payload.name}` : ''}!</p><p>Смени паролата си от този линк:</p><p><a href="${resetUrl}">${resetUrl}</a></p><p>Ако не си поискал смяна на парола, игнорирай това съобщение.</p>`,
      type: 'password_reset',
    });
  }
  ...
  async sendPasswordChanged(payload: { email: string; name?: string }) {
    return this.sendAccountMail({
      ...
      html: `<p>Здравей${payload.name ? `, ${payload.name}` : ''}!</p><p>Паролата ти в Bricky беше сменена успешно.</p><p>Ако това не си бил ти, свържи се с поддръжката веднага.</p>`,
