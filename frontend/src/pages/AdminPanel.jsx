...
export default function AdminPanel() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const queuePath = tab === "audit"
      ? `/admin/audit-logs?action=${encodeURIComponent(status)}&q=${encodeURIComponent(query)}&page=${page}&limit=25`
      : `/admin/${tab}?status=${encodeURIComponent(status)}&q=${encodeURIComponent(query)}&page=${page}&limit=25`;
    ...
    setItems(tab === "users" ? rows.filter((item) => item.accountStatus === status && (!query || JSON.stringify(item).toLowerCase().includes(query.toLowerCase()))) : rows);
    setLoading(false);
  ...
  const openDetails = async (item) => {
    ...
    const path = tab === "media"
      ? `/admin/media/${item.source || "request"}/${item.id}`
      : `/admin/${tab}/${item.id}`;
  ...
  return <section className="bricky-page min-h-[calc(100vh-78px)] py-10"><div className="bricky-container space-y-7">
    ...
    <div className="flex flex-wrap gap-2 border-b border-slate-700 pb-3">{[["requests","Заявки"],["media","Снимки"],["workers","Майстори"],["reviews","Ревюта"],["users","Акаунти"],["audit","Одит"]].map(([key,name]) => <button key={key} onClick={() => { setTab(key); setStatus(key === "users" ? "active" : key === "audit" ? "" : "pending_review"); setPage(1); }} className={`rounded-lg px-5 py-3 font-bold ${tab === key ? "bg-green-500 text-slate-950" : "bg-slate-800"}`}>{name}</button>)}</div>
    <div className="bricky-card grid gap-3 p-4 md:grid-cols-[1fr_240px]">
      ...
      <select value={status} onChange={(event) => { setStatus(event.target.value); setPage(1); }} className="rounded-xl border border-slate-600 bg-[#0d1728] px-4 py-3 text-white">
        {tab === "users" ? <><option value="active">Активни</option><option value="suspended">Спрени</option></> : tab === "audit" ? <><option value="">Всички действия</option><option value="approved">Одобряване</option><option value="rejected">Отхвърляне</option><option value="hidden">Скриване</option><option value="edited">Редакция</option><option value="deleted">Изтриване</option><option value="suspend">Спиране на акаунт</option><option value="activate">Активиране на акаунт</option></> : <><option value="pending_review">Чака преглед</option><option value="approved">Одобрено</option><option value="rejected">Отхвърлено</option><option value="hidden">Скрито</option></>}
      </select>
    </div>
    {loading ? <p>Зареждане...</p> : items.length === 0 ? <div className="bricky-card p-10 text-center text-slate-300"><ShieldCheck className="mx-auto mb-3 text-green-300"/>Няма съдържание за този филтър.</div> : <div className="grid gap-4">{items.map(item => <QueueItem key={`${item.source || tab}-${item.id}`} item={item} type={tab} onAction={moderate} onView={() => openDetails(item)}/>)}</div>}
...
function QueueItem({ item, type, onAction, onView }) { return <article className="bricky-card flex flex-col gap-5 p-5 lg:flex-row lg:items-center">
  {type === "media" && <img src={item.thumbnailUrl || item.url} alt="Съдържание за модерация" className="h-32 w-48 rounded-lg object-cover"/>}
  <div className="min-w-0 flex-1"><span className={`rounded-full px-3 py-1 text-sm font-bold ${type === "users" && item.accountStatus === "active" ? "bg-green-500/15 text-green-200" : type === "users" ? "bg-red-500/15 text-red-200" : type === "audit" ? "bg-cyan-500/15 text-cyan-200" : "bg-amber-500/15 text-amber-200"}`}>{type === "users" ? item.accountStatus === "active" ? "Активен" : "Спрян" : type === "audit" ? item.action : "Чака преглед"}</span><h2 className="mt-3 text-xl font-black">{queueTitle(item, type)}</h2><p className="mt-2 text-slate-300">{item.reason || item.description || item.comment || item.email || item.name || item.url || item.city || "Без допълнително описание"}</p></div>
...
function queueTitle(item, type) {
  ...
  if (type === "users") return item.fullName || item.name || item.email || `Акаунт #${item.id}`;
  if (type === "audit") return `${item.entityType} #${item.entityId} · админ #${item.adminUserId}`;
