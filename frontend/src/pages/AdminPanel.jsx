  ...
  return <section className="bricky-page min-h-[calc(100vh-78px)] py-10"><div className="bricky-container space-y-7">
    ...
    <div className="bricky-card grid gap-3 p-4 md:grid-cols-[1fr_240px]">
      ...
      <select value={status} onChange={(event) => { setStatus(event.target.value); setPage(1); }} className="rounded-xl border border-slate-600 bg-[#0d1728] px-4 py-3 text-white">
        {tab === "users" ? <><option value="active">Активни</option><option value="suspended">Спрени</option></> : <><option value="pending_review">Чака преглед</option><option value="approved">Одобрено</option><option value="rejected">Отхвърлено</option><option value="hidden">Скрито</option></>}
      </select>
    </div>
...
function QueueItem({ item, type, onAction, onView }) { return <article className="bricky-card flex flex-col gap-5 p-5 lg:flex-row lg:items-center">
  ...
  <div className="min-w-0 flex-1"><span className={`rounded-full px-3 py-1 text-sm font-bold ${type === "users" && item.accountStatus === "active" ? "bg-green-500/15 text-green-200" : type === "users" ? "bg-red-500/15 text-red-200" : "bg-amber-500/15 text-amber-200"}`}>{type === "users" ? item.accountStatus === "active" ? "Активен" : "Спрян" : "Чака преглед"}</span><h2 className="mt-3 text-xl font-black">{queueTitle(item, type)}</h2><p className="mt-2 text-slate-300">{item.description || item.comment || item.email || item.name || item.url || item.city || "Без допълнително описание"}</p></div>
  <div className="flex flex-wrap gap-2"><button onClick={onView} className="rounded-lg border border-slate-600 px-4 py-3 font-bold">Детайли</button>{type === "users" ? item.accountStatus === "suspended" ? <button onClick={() => onAction(item.id,"activate")} className="bricky-button-primary"><Check size={17}/> Активирай</button> : <button onClick={() => onAction(item.id,"suspend")} className="rounded-lg bg-red-500/15 px-4 py-3 font-bold text-red-200"><EyeOff size={17} className="inline"/> Спри акаунта</button> : <><button onClick={() => onAction(item.id,"approved")} className="bricky-button-primary"><Check size={17}/> Одобри</button><button onClick={() => onAction(item.id,"rejected")} className="rounded-lg bg-red-500/15 px-4 py-3 font-bold text-red-200"><X size={17} className="inline"/> Отхвърли</button><button onClick={() => onAction(item.id,"hidden")} className="rounded-lg bg-slate-700 px-4 py-3 font-bold"><EyeOff size={17} className="inline"/> Скрий</button></>}</div>
</article>; }
