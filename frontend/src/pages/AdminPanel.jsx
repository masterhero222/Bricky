...
export default function AdminPanel() {
  ...
  const [summary, setSummary] = useState({ pendingRequests: 0, pendingMedia: 0 });
  ...
  const moderate = async (id, action) => {
    ...
    await apiPost(`/admin/${tab}/${id}/${action}`, { reason }); await load();
  };
  return <section className="bricky-page min-h-[calc(100vh-78px)] py-10"><div className="bricky-container space-y-7">
    ...
    <div className="grid gap-4 sm:grid-cols-2"><Stat title="Заявки за преглед" value={summary.pendingRequests}/><Stat title="Снимки за преглед" value={summary.pendingMedia}/></div>
    <div className="flex gap-2 border-b border-slate-700 pb-3">{[["requests","Заявки"],["media","Снимки"]].map(([key,name]) => <button key={key} onClick={() => setTab(key)} className={`rounded-lg px-5 py-3 font-bold ${tab === key ? "bg-green-500 text-slate-950" : "bg-slate-800"}`}>{name}</button>)}</div>
...
function QueueItem({ item, type, onAction }) { return <article className="bricky-card flex flex-col gap-5 p-5 lg:flex-row lg:items-center">
  ...
  <div className="min-w-0 flex-1"><span className="rounded-full bg-amber-500/15 px-3 py-1 text-sm font-bold text-amber-200">Чака преглед</span><h2 className="mt-3 text-xl font-black">{type === "requests" ? `#${item.id} · ${item.category || "Заявка"}` : `Снимка #${item.id}`}</h2><p className="mt-2 text-slate-300">{item.description || item.name || item.url}</p></div>
