    ...
    <header className="fixed inset-x-0 top-0 z-50 h-[78px] border-b border-slate-400/15 bg-[#08111f]/85 backdrop-blur-xl">
      <div className="bricky-container flex h-full items-center justify-between gap-5">
        ...
        >
          ...
          {role === 'worker' && (
            <NavLink to="/repair-map" className={navClass}>
              Карта
            </NavLink>
          )}
          ...
          {!role ? (
            <>
              <button
                ...
                className="min-h-11 rounded-xl border border-slate-400/15 bg-slate-800/70 px-5 font-bold text-slate-100 hover:bg-slate-700/80"
              >
                Вход
              </button>
              <button
                onClick={() => navigate('/auth/register')}
                className="bricky-button-secondary !min-h-11"
              >
                Регистрация
              </button>
            </>
            ...
            {!role ? (
              <div className="mt-3 grid grid-cols-2 gap-3">
                <button
                  ...
                  className="rounded-xl border border-slate-400/15 bg-slate-800 px-4 py-3"
                >
                  Вход
                </button>
                <button
