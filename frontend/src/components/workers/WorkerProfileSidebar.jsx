...
export default function WorkerProfileSidebar({ activeTab, onSelect }) {
  ...
  const activeLabel = WORKER_PROFILE_NAV_ITEMS.find(([key]) => key === activeTab)?.[1] || "Меню";
  ...
  const navigation = (mobile = false) => (
    <nav className="flex flex-col gap-2 px-5 text-sm">
      ...
      <div className={mobile ? "mt-2" : "mt-6"}><LogoutButton /></div>
    </nav>
  );
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-0 top-[78px] z-40 border-b border-cyan-400/20 bg-[#071827] shadow-xl shadow-black/45 lg:hidden">
        <button type="button" onClick={() => setMobileOpen((open) => !open)} className="flex min-h-14 w-full items-center justify-between px-5 font-bold text-white" aria-expanded={mobileOpen}>
          <span className="flex items-center gap-2"><Menu size={19} />{activeLabel}</span>
          {mobileOpen ? <X size={20} /> : <ChevronDown size={20} />}
        </button>
        {mobileOpen && <div className="max-h-[calc(100vh-134px)] overflow-y-auto border-t border-cyan-400/15 bg-[#071827] py-3">{navigation(true)}</div>}
      </div>
    </>
  );
}
